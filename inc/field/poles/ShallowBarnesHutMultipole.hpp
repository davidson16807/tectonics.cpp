...
namespace field
{
	...
	/*
	`ShallowBarnesHutMultipole` represents a mathematical field of arbitrary dimensionality
	...
	`ShallowBarnesHutMultipole` is an implementation of the Barnes-Hut algorithm.
	`ShallowBarnesHutMultipole` can be used to efficiently implement inverse distance weighting, 
	...
	In boid simulation, multiple `ShallowBarnesHutMultipole`s are used, and weights associated with a boid are either uniform or represent heading.

	`ShallowBarnesHutMultipole` represents a field using an orthtree 
	that is sufficiently shallow so as to be stored in contiguous memory as a std::vector.
		...
		*/
		[[nodiscard]] inline vector operator()(const vector& position ) const
		{
			...
			for (id level = id(1); level < level_count; ++level)
			{
				...
				orthant_id = orthants.memory_id(orthant);
				nesting_id = orthant_count * nesting_id + orthant_id;
				const Monopole<scalar,vector>& monopole = orthtree[first_id_for_level[level] + nesting_id];
