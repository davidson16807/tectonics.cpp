...
namespace generic{
  /*
  `GenericText` is a category class that serves as a wrapper for basic text functionality within `std`.
  Code that uses the functionality can reference the functionality using an interface or template, 
  so the same code can be repurposed to work with other data types by writing an analogous *Text class for that data type.
