...
namespace math {
    ...
    /*
    A `Train` is a sum of Railcar functions, contiguously arranged, each multiplied by functions of an arbitrary type F.
    */
    ...
    struct Train {
        ...
        Train(const Train<T,F>& p) : terms(p.terms) 
        {
        }
        explicit Train(const std::vector<F> terms_) : terms(terms_) 
        {
        }
        explicit Train(std::initializer_list<F> ts)
        {
        ...
        template<typename G>
        Train<T,F>& operator+=(const Train<T,G>& q)
        {
    ...
    template<typename T, typename F, typename G>
    constexpr auto operator+(const Train<T,F>& f, const G g)
    {
        Train<T,F> y(f);
        y += F(g);
        return y;
    ...
    template<typename T, typename F, typename G>
    constexpr auto operator+(const G g, const Train<T,F>& f)
    {
        Train<T,F> y(f);
        y += F(g);
        return y;
    ...
    template<typename T, typename F>
    constexpr auto operator-(const Train<T,F>& g, const Train<T,F>& f)
    {
        Train<T,F> y(f);
        y *= T(-1);
        y += g;
    ...
    constexpr auto operator+(const Train<T,F>& p, const Train<T,Qlo,Qhi>& q)
    {
        ...
        return simplify(Train<T,std::min(Plo,Qlo),std::max(Phi,Qhi)>(terms));
    }
    ...
    constexpr auto operator*(const Train<T,F>& p, const Train<T,Qlo,Qhi>& q)
    {
        ...
        return simplify(Train<T,Plo+Qlo,Phi+Qhi>(terms));
    }
    ...
    template<typename T, typename F>
    constexpr auto operator+(const Train<T,F>& p, const Shifting<T> f)
    {
        return p + Polynomial<T,0,1>(f);
    }
    template<typename T, typename F>
    constexpr auto operator+(const Shifting<T> f, const Train<T,F>& p)
    {
    ...
    template<typename T, typename F>
    constexpr auto operator+(const Train<T,F>& p, const Scaling<T> f)
    {
        return p + Polynomial<T,1,1>(f);
    }
    template<typename T, typename F>
    constexpr auto operator+(const Scaling<T> f, const Train<T,F>& p)
    {
    ...
    template<typename T, typename F>
    constexpr auto operator+(const Train<T,F>& p, const Identity<T> e)
    {
        return p + Polynomial<T,1,1>(e);
    }
    template<typename T, typename F>
    constexpr auto operator+(const Identity<T> e, const Train<T,F>& p)
    {
    ...
    template<typename T, typename F, int Qlo, int Qhi>
    constexpr Train<T,std::min(Plo*Qlo,Phi*Qhi),std::max(Plo*Qlo,Phi*Qhi)> compose(const Train<T,F>& p, const Polynomial<T,Qlo,Qhi> q)
    {
    ...
    template<typename T, typename F>
    constexpr Train<T,F> compose(const Train<T,F>& p, const Scaling<T> g)
    {
    ...
    template<typename T, typename F>
    constexpr Train<T,F> compose(const Train<T,F>& p, const Shifting<T> g)
    {
        using PiecewisePolynomial = F;
        std::vector<PiecewisePolynomial> terms;
        for (std::size_t i=0; i<p.terms.size(); i++)
    ...
    template<typename T, typename F>
    constexpr Train<T,F> compose(const Train<T,F>& p, const Identity<T> e)
    {
        return p;
    }
    ...
    template<typename T, typename F>
    constexpr Train<T,Plo-1,Phi-1> derivative(const Train<T,F>& p)
    {
    ...
    */
    template<typename T, typename F>
    T derivative(const Train<T,F>& p, const T x)
    {
        T dydx(0.0f);
        for (std::size_t i=0; i<p.terms.size(); i++)
        {
    ...
    */
    template<typename T, typename F>
    T integral(const Train<T,F>& p, const T x)
    {
        T I(0.0f);
        for (std::size_t i=0; i<p.terms.size(); i++)
        {
    ...
    template<typename T, typename F>
    T integral(const Train<T,F>& p, const T lo, const T hi)
    {
        T I(0.0f);
        for (std::size_t i=0; i<p.terms.size(); i++)
        {
    ...
    template<typename T, typename F>
    constexpr Train<T,Plo+1,Phi+1> integral(const Train<T,F>& p)
    {
    ...
    template<typename T, typename F>
    constexpr Train<T,F> restriction(const Train<T,F>& p, const T lo, const T hi)
    {
    ...
    template<typename T, typename F>
    constexpr Train<T,Plo*2,Phi*2> square(const Train<T,F>& p){
        return p*p;
        // Train<T,F> q = simplify(p);
        ...
        // }
        // return Train<T,Plo*2,Phi*2>(terms);
    }
    ...
    */
    ...
    template<typename T, typename F, int Qlo, int Qhi>
    constexpr T distance(const Train<T,F>& p, const Polynomial<T,Qlo,Qhi> q, const T lo, const T hi)
    {
        return std::sqrt(std::max(T(0), integral(square(p-q), lo, hi))) / (hi-lo);
    }
    template<typename T, typename F, int Qlo, int Qhi>
    constexpr T distance(const Polynomial<T,Plo,Phi> p, const Train<T,Qlo,Qhi> q, const T lo, const T hi)
    {
        return std::sqrt(std::max(T(0), integral(square(p-q), lo, hi))) / (hi-lo);
    }
    template<typename T, typename F>
    constexpr T distance(const Train<T,F>& p, const T k, const T lo, const T hi)
    {
        return std::sqrt(std::max(T(0), integral(square(p-k), lo, hi))) / (hi-lo);
    }
    template<typename T, typename F>
    constexpr T distance(const T k, const Train<T,Plo, Phi> p, const T lo, const T hi)
    {
        return std::sqrt(std::max(T(0), integral(square(p-k), lo, hi))) / (hi-lo);
    }
    ...
    */
    ...
    template<typename T, typename F, int Qlo, int Qhi>
    constexpr T dot(const Train<T,F>& p, const Polynomial<T,Qlo,Qhi> q, const T lo, const T hi)
    {
        return integral(p*q, lo, hi);
    }
    template<typename T, typename F, int Qlo, int Qhi>
    constexpr T dot(const Polynomial<T,Plo,Phi> p, const Train<T,Qlo,Qhi> q, const T lo, const T hi)
    {
        return integral(p*q, lo, hi);
    }
    template<typename T, typename F>
    constexpr T dot(const Train<T,F>& p, const T k, const T lo, const T hi)
    {
        return integral(p*k, lo, hi);
    }
    template<typename T, typename F>
    constexpr T dot(const T k, const Train<T,Plo, Phi> p, const T lo, const T hi)
    {
        return integral(p*k, lo, hi);
    }
    ...
    */
    ...
    template<typename T, typename F, int Qlo, int Qhi>
    constexpr T similarity(const Train<T,F>& p, const Polynomial<T,Qlo,Qhi> q, const T lo, const T hi)
    {
        return dot(p,q,lo,hi) / (length(p,lo,hi)*length(q,lo,hi));
    }
    template<typename T, typename F, int Qlo, int Qhi>
    constexpr T similarity(const Polynomial<T,Plo,Phi> p, const Train<T,Qlo,Qhi> q, const T lo, const T hi)
    {
        return dot(p,q,lo,hi) / (length(p,lo,hi)*length(q,lo,hi));
    }
    template<typename T, typename F>
    constexpr T similarity(const Train<T,F>& p, const T k, const T lo, const T hi)
    {
        return dot(p,k,lo,hi) / (length(p,lo,hi)*length(k,lo,hi));
    }
    template<typename T, typename F>
    constexpr T similarity(const T k, const Train<T,Plo, Phi> p, const T lo, const T hi)
    {
        return dot(p,k,lo,hi) / (length(p,lo,hi)*length(k,lo,hi));
    }
    ...
    /*
    template<typename T, typename F, int Qlo, int Qhi>
    constexpr Train<T,std::min(Plo,Qlo),std::max(Phi,Qhi)> max(const Polynomial<T,Plo,Phi> p, const Polynomial<T,Qlo,Qhi> q)
    {
        ...
        T lo, hi, x;
        Train<T,std::min(Plo,Qlo),std::max(Phi,Qhi)> spline;
    ...
    template<typename T, typename F, int Qlo, int Qhi>
    constexpr Train<T,std::min(Plo,Qlo),std::max(Phi,Qhi)> min(const Polynomial<T,Plo,Phi> p, const Polynomial<T,Qlo,Qhi> q)
    {
        auto candidates = solve(p-q, 0.0f);
        std::vector<T> solutions;
        reals(candidates.begin(), candidates.end(), std::back_inserter(solutions), 1e-7);
    ...
    template<typename T, typename F>
    constexpr Train<T,std::min(Plo,0),std::max(Phi,0)> max(const Polynomial<T,Plo,Phi> p, const T k)
    {
        return max(p,Polynomial<T,0,0>{k});
    }

    template<typename T, typename F>
    constexpr Train<T,std::min(Plo,0),std::max(Phi,0)> max(const T k, const Polynomial<T,Plo, Phi> p)
    {
        return max(p,Polynomial<T,0,0>{k});
    }
