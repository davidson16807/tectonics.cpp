...
#include <math/expression/ArbitraryDegreeRational.hpp>  


TEST_CASE( "ArbitraryDegreeRational arithmetic purity", "[math]" ) {
...
TEST_CASE( "ArbitraryDegreeRational arithmetic identity", "[math]" ) {
    ...
    auto zero = math::ArbitraryDegreeRational<double,std::int8_t>(math::Polynomial<double,0,0>{0.0f}, math::Polynomial<double,0,0>{1.0});
    auto one  = math::ArbitraryDegreeRational<double,std::int8_t>(math::Polynomial<double,0,0>{1.0}, math::Polynomial<double,0,0>{1.0});
...
TEST_CASE( "ArbitraryDegreeRational arithmetic commutativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -100.0f;
...
TEST_CASE( "ArbitraryDegreeRational arithmetic associativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
...
TEST_CASE( "ArbitraryDegreeRational arithmetic distributivity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -3.0f;
...
TEST_CASE( "ArbitraryDegreeRational/scalar arithmetic purity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
...
TEST_CASE( "ArbitraryDegreeRational/scalar arithmetic identity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double lo = -1000.0f;
    const double hi =  1000.0f;
...
TEST_CASE( "ArbitraryDegreeRational/scalar arithmetic commutativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `lo/hi` variables are used as bounds to a square integral 
    // that is used to calculate deviation from the correct output.
    const double lo = -1000.0f;
...
TEST_CASE( "ArbitraryDegreeRational/scalar arithmetic associativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double midlo = -100.0f;
    const double midhi =  100.0f;
...
TEST_CASE( "ArbitraryDegreeRational/scalar arithmetic distributivity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -100.0f;
...
TEST_CASE( "ArbitraryDegreeRational/monomial arithmetic purity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double lo = -30.0f;
...
TEST_CASE( "ArbitraryDegreeRational/monomial arithmetic identity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double lo = -1000.0f;
    const double hi =  1000.0f;
...
TEST_CASE( "ArbitraryDegreeRational/monomial arithmetic commutativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -100.0f;
...
TEST_CASE( "ArbitraryDegreeRational/monomial arithmetic associativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double midlo = -100.0f;
    const double midhi =  100.0f;
...
TEST_CASE( "ArbitraryDegreeRational/monomial arithmetic distributivity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -3.0f;
...
TEST_CASE( "ArbitraryDegreeRational/Shifting arithmetic purity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `lo/hi` variables are used as bounds to a square integral 
    // that is used to calculate deviation from the correct output.
    const double lo = -1000.0f;
...
TEST_CASE( "ArbitraryDegreeRational/Shifting arithmetic commutativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `lo/hi` variables are used as bounds to a square integral 
    // that is used to calculate deviation from the correct output.
    const double lo = -1000.0f;
...
TEST_CASE( "ArbitraryDegreeRational/Shifting arithmetic associativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double midlo = -100.0f;
    const double midhi =  100.0f;
...
TEST_CASE( "ArbitraryDegreeRational/Shifting arithmetic distributivity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -100.0f;
...
TEST_CASE( "ArbitraryDegreeRational/Scaling arithmetic purity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `lo/hi` variables are used as bounds to a square integral 
    // that is used to calculate deviation from the correct output.
    const double lo = -1000.0f;
...
TEST_CASE( "ArbitraryDegreeRational/Scaling arithmetic identity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double lo = -1000.0f;
    const double hi =  1000.0f;
...
TEST_CASE( "ArbitraryDegreeRational/Scaling arithmetic commutativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `lo/hi` variables are used as bounds to a square integral 
    // that is used to calculate deviation from the correct output.
    const double lo = -1000.0f;
...
TEST_CASE( "ArbitraryDegreeRational/Scaling arithmetic associativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double midlo = -100.0f;
    const double midhi =  100.0f;
...
TEST_CASE( "ArbitraryDegreeRational/Scaling arithmetic distributivity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -100.0f;
