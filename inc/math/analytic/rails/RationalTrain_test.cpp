...
#include "RationalTrain.hpp"


TEST_CASE( "RationalTrain arithmetic purity", "[math]" ) {
    ...
    // laurent polynomials
    math::PolynomialTrain<double,-2,2> t2 = math::Train<double,P2>(std::vector<P2>{p2,P2(p1),p3},std::vector<double>{-oo,-2.0,2.0,oo});
    // all polynomials
    math::PolynomialTrain<double,-2,2> t3 = math::Train<double,P2>(std::vector<P2>{p3,P2(p0),p1},std::vector<double>{-oo,-3.0,3.0,oo});
...
TEST_CASE( "RationalTrain arithmetic identity", "[math]" ) {
    ...
    auto zero = math::Rational<double,0,0,0,0>(math::Polynomial<double,0,0>{0.0f}, math::Polynomial<double,0,0>{1.0});
    auto one  = math::Rational<double,0,0,0,0>(math::Polynomial<double,0,0>{1.0}, math::Polynomial<double,0,0>{1.0});
...
TEST_CASE( "RationalTrain arithmetic commutativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -100.0f;
...
TEST_CASE( "RationalTrain arithmetic associativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
...
TEST_CASE( "RationalTrain arithmetic distributivity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -3.0f;
...
TEST_CASE( "RationalTrain/scalar arithmetic purity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
...
TEST_CASE( "RationalTrain/scalar arithmetic identity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double lo = -1000.0f;
    const double hi =  1000.0f;
...
TEST_CASE( "RationalTrain/scalar arithmetic commutativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `lo/hi` variables are used as bounds to a square integral 
    // that is used to calculate deviation from the correct output.
    const double lo = -1000.0f;
...
TEST_CASE( "RationalTrain/scalar arithmetic associativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double midlo = -100.0f;
    const double midhi =  100.0f;
...
TEST_CASE( "RationalTrain/scalar arithmetic distributivity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -100.0f;
...
TEST_CASE( "RationalTrain/monomial arithmetic purity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double lo = -30.0f;
...
TEST_CASE( "RationalTrain/monomial arithmetic identity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double lo = -1000.0f;
    const double hi =  1000.0f;
...
TEST_CASE( "RationalTrain/monomial arithmetic commutativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -100.0f;
...
TEST_CASE( "RationalTrain/monomial arithmetic associativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double midlo = -100.0f;
    const double midhi =  100.0f;
...
TEST_CASE( "RationalTrain/monomial arithmetic distributivity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -3.0f;
...
TEST_CASE( "RationalTrain/Shifting arithmetic purity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `lo/hi` variables are used as bounds to a square integral 
    // that is used to calculate deviation from the correct output.
    const double lo = -1000.0f;
...
TEST_CASE( "RationalTrain/Shifting arithmetic commutativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `lo/hi` variables are used as bounds to a square integral 
    // that is used to calculate deviation from the correct output.
    const double lo = -1000.0f;
...
TEST_CASE( "RationalTrain/Shifting arithmetic associativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double midlo = -100.0f;
    const double midhi =  100.0f;
...
TEST_CASE( "RationalTrain/Shifting arithmetic distributivity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -100.0f;
...
TEST_CASE( "RationalTrain/Scaling arithmetic purity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `lo/hi` variables are used as bounds to a square integral 
    // that is used to calculate deviation from the correct output.
    const double lo = -1000.0f;
...
TEST_CASE( "RationalTrain/Scaling arithmetic identity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double lo = -1000.0f;
    const double hi =  1000.0f;
...
TEST_CASE( "RationalTrain/Scaling arithmetic commutativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `lo/hi` variables are used as bounds to a square integral 
    // that is used to calculate deviation from the correct output.
    const double lo = -1000.0f;
...
TEST_CASE( "RationalTrain/Scaling arithmetic associativity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    const double midlo = -100.0f;
    const double midhi =  100.0f;
...
TEST_CASE( "RationalTrain/Scaling arithmetic distributivity", "[math]" ) {
    const double threshold = std::numeric_limits<double>::epsilon();
    // `mid*` variables are used when the degree of a polynomial is so large 
    // that a square integral of it will produce nans for all but the smallest input.
    const double midlo = -100.0f;
