...
namespace many
{
	...
	std::string to_string(const std::vector<glm::vec<2,T,Q>>& a, const int line_char_width = 80)
	{
		std::vector<T> length_a(a.size());
		length(a, length_a);
	...
	template <typename T, glm::qualifier Q>
	std::string to_string(const std::vector<glm::vec<3,T,Q>>& a, const glm::mat<3,2,T,Q>& basis, const int line_char_width = 80)
	{
		std::vector<glm::vec<2,T,Q>> a2d(a.size());
		mult(basis, a, a2d);
