...
`light::MultipoleInstellation` instantiates a field that represents the cross-spectrum "top-of-atmosphere" irradiance from an arbitrary number of light sources
The light sources are modeled as point light sources where irradiance drops of as the inverse square of distance.

`light::MultipoleInstellation` correctly models several aspects of the problem:
...
`light::MultipoleInstellation` introduces several assumptions that are either intrinsic to the class or will be addressed in time:
...
* logic for occlusion by any body aside from the planet at the origin
    ...
    Our climate modeling use case does not require calculating instellation with the precision
    ...
    This can be done before constructing the MultipoleInstellation, when generating samples.
...
namespace air
{
    ...
    [[nodiscard]] auto MultipoleInstellation (const LightExposures& exposures)
    {
        ...
        Multipole instellation;
        for (const auto& exposure : exposures)
        {
            instellation.add(s4pi_inverse * exposure.source.luminosity(), exposure.offset);
        }
        return instellation;
