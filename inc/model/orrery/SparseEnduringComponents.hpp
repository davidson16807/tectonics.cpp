...
/*
`SparseEnduringComponents` represents a table of components within an Entity-Component-System ("ECS") pattern
...
so avoid `SparseEnduringComponents` if sorting by entity id will better exploit cache memory in traversals involving multiple component types.

`SparseEnduringComponents` is best suited if components are expected to be added or removed frequently,
...
class SparseEnduringComponents
{
...
public:
	...
	/*
	Create an empty `SparseEnduringComponents<id,Component>` instance 
	*/
	SparseEnduringComponents():
		components(),
		entity_of_index(),
		index_of_entity(),
		size(0)
	...
	/*
	Create a `SparseEnduringComponents<id,Component>` instance from the vector `components`
	where each components corresponds to a unique and newly constructed entity
	whose id is equal to the component's index.
	*/
	SparseEnduringComponents(const std::vector<Component>& components_):
	...
	/*
	Create a `SparseEnduringComponents<id,Component>` 
	that has memory preallocated to serve a given number of entities.
	*/
	SparseEnduringComponents(const id& entity_count):
		components(),
		entity_of_index(),
		index_of_entity(),
		size(0)
