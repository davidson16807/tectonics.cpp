...
TEST_CASE( "Series<T> abs purity", "[whole]" ) {
    auto a = series::interleave({-1,1,-2,2,3});
...
TEST_CASE( "Series<T> abs/sign consistency", "[whole]" ) {
    auto a = series::interleave({-1,1,-2,2,3});
    auto abs_a = series::interleave({0,0,0,0,0});
    auto sign_a = series::interleave({0,0,0,0,0});
    auto a2 = series::interleave({0,0,0,0,0});
...
TEST_CASE( "Series<T> floor/fract consistency", "[whole]" ) {
    auto a = series::interleave({-1,1,-2,2,3});
    auto floor_a = series::interleave({0,0,0,0,0});
    auto fract_a = series::interleave({0,0,0,0,0});
    auto a2 = series::interleave({0,0,0,0,0});
...
TEST_CASE( "Series<T> mod purity", "[whole]" ) {
    auto a = series::interleave({-1,1,-2,2,3});
    auto b = series::interleave({1,2,3,4,5});
    auto out1 = series::interleave({0,0,0,0,0});
    auto out2 = series::interleave({0,0,0,0,0});
...
TEST_CASE( "Series<T> mod/fract consistency", "[whole]" ) {
    auto a = series::interleave({1.0,2.1,3.1,4.2,5.3});
...
TEST_CASE( "Series<T> min purity", "[whole]" ) {
    auto a = series::interleave({3,2,-2,1,-1});
    auto b = series::interleave({1,2,3,4,5});
    auto out1 = series::interleave({0,0,0,0,0});
    auto out2 = series::interleave({0,0,0,0,0});
...
TEST_CASE( "Series<T> min decreasing", "[whole]" ) {
    auto a = series::interleave({3,2,-2,1,-1});
    auto b = series::interleave({1,2,3,4,5});
    auto c = series::interleave({10,0,0,0,-10});
    auto ab = series::interleave({0,0,0,0,0});
    auto abc = series::interleave({0,0,0,0,0});
...
TEST_CASE( "Series<T> min idempotence", "[whole]" ) {
    ...
    auto b = series::interleave({1,2,3,4,5});
    auto ab = series::interleave({0,0,0,0,0});
    auto abb = series::interleave({0,0,0,0,0});
...
TEST_CASE( "Series<T> min/max/clamp consistency", "[whole]" ) {
    ...
    auto lo = series::interleave({-1,1,-2,2,3});
    auto hi = series::interleave({7,6,5,4,3});
    auto alo = series::interleave({0,0,0,0,0});
    auto alohi1 = series::interleave({0,0,0,0,0});
    auto alohi2 = series::interleave({0,0,0,0,0});
...
TEST_CASE( "Series<T> string cast correctness", "[whole]" ) {
    auto a = series::interleave({1,2,3,4,5,6});
    auto b = series::interleave({1,1,2,3,5,8});
    auto c = series::interleave<float>({4,8,3,8,2,4,5,9,8,2,3,5,2,1,3,3,3,1,6,1,
    ...
    SECTION("to_string() must render correct representation"){
        ...
        REQUIRE_THAT(whole::to_string(series::interleave({INFINITY})), Catch::Contains("∞"));
        REQUIRE_THAT(whole::to_string(series::interleave({std::sqrt(-1)})), Catch::Contains("N"));
    }
}
...
TEST_CASE( "Series<T> string cast representation", "[whole]" ) {
    ...
    SECTION("to_string() must capture appreciable differences in the content of Series<T>"){
        for(std::size_t i=0; i<a.size(); ++i){
            auto b = series::interleave({1,2,3,4,5,6});
            b[i] = -1;
            std::string stra = whole::to_string(a);
            std::string strb = whole::to_string(b);
            CHECK(stra != strb);
    ...
    SECTION("to_string() must capture appreciable differences in the length of Series<T>"){
        auto c = series::interleave({1,2,3,4,5,6,7});
        std::string stra = whole::to_string(a);
        std::string strc = whole::to_string(c);
        CHECK(stra != strc);
    }
