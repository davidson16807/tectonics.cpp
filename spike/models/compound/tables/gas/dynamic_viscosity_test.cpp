...
TEST_CASE( "gaseous dynamic_viscosity order of magnitude", "[table]" ) {
    SECTION("Properties of compounds should not fall outside orders of magnitude for known values")
    {
        for (si::pressure<double> p = 1.0*si::pascal; p <= 100000000.0*si::pascal; p*=10.0)
        {
            for (si::temperature<double> T = 3.0*si::kelvin; T <= si::solar_temperature; T*=3.0)
            {
                ...
                {
                    if (compound::tables::dynamic_viscosity_as_gas.count(i) > 0) {
                        auto x = compound::tables::dynamic_viscosity_as_gas[i](p,T);
