...
TEST_CASE( "liquid vapor_pressure order of magnitude", "[table]" ) {
    SECTION("Properties of compounds should not fall outside orders of magnitude for known values")
    {
        for (int i = 0; i<compound::tables::ids::count; i++)
        {
            if (compound::tables::vapor_pressure_as_liquid.count(i) > 0) 
            {
                for (si::temperature<double> T = 3.0*si::kelvin; T <= 100.0*si::kelvin; T*=3.0)
                {
                    auto x = compound::tables::vapor_pressure_as_liquid[i](T);
