...
TEST_CASE( "liquid dynamic_viscosity order of magnitude", "[table]" ) {
    SECTION("Properties of compounds should not fall outside orders of magnitude for known values")
    {
        for (int i = 0; i<compound::tables::ids::count; i++)
        {
            ...
            {
                if (compound::tables::dynamic_viscosity_as_liquid.count(i) > 0) {
                    auto x = compound::tables::dynamic_viscosity_as_liquid[i](T);
