...
TEST_CASE( "PartlyKnownLiquid to_string() appreciable difference preservation", "[field]" ) {
	compound::phase::PartlyKnownLiquid nudge_isobaric_specific_heat_capacity = dummy_liquid; nudge_isobaric_specific_heat_capacity .isobaric_specific_heat_capacity = std::monostate();
	...
	compound::phase::PartlyKnownLiquid nudge_extinction_coefficient          = dummy_liquid; nudge_extinction_coefficient          .extinction_coefficient          = std::monostate();
	
	SECTION("Appreciable differences in input are preserved within output")
	{
		CHECK(compound::phase::to_string(nudge_isobaric_specific_heat_capacity) != compound::phase::to_string(dummy_liquid));
