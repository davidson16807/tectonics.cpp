...
#include "SplineRelation.hpp"

namespace compound {
namespace relation {

	template <int M1, int KG1, int S1, int K1, int MOL1, int A1, int CD1, typename Tx, typename Ty, int Plo, int Phi>
	std::string to_string(const SplineRelation<Tx, si::units<M1,KG1,S1,K1,MOL1,A1,CD1,Ty>, Plo, Phi>& known, Tx parameter)
	{
		return si::to_string(known(parameter));
	}

	template <typename Tx, typename Ty, int Plo, int Phi>
	std::string to_string(const SplineRelation<Tx, Ty, Plo, Phi>& known, Tx parameter)
	{
