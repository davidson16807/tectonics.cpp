...
namespace strata
{
	...
	*/
	...
	{
		float mass; 
		std::array<std::uint8_t, GrainType::count> grain_type_bin_relative_volume;
	...
	public:
		...
		void decompress(StratumMassPool& output) const
		{
            ...
            for (std::size_t i=0; i<GrainType::count; i++)
            {
                total_relative_volume += grain_type_bin_relative_volume[i];
            }
            for (std::size_t i=0; i<GrainType::count; i++)
            {
                output.grain_type_bin_fractional_volume[i] = grain_type_bin_relative_volume[i] / total_relative_volume;
            }
		}
		void compress(const StratumMassPool& input)
		{
            ...
            for (std::size_t i=0; i<GrainType::count; i++)
            {
                grain_type_relative_bin_count_max = std::max(grain_type_relative_bin_count_max, input.grain_type_bin_fractional_volume[i]);
            }
            for (std::size_t i=0; i<GrainType::count; i++)
            {
                grain_type_bin_relative_volume[i] = 
                    std::clamp(input.grain_type_bin_fractional_volume[i] * 255.0f / grain_type_relative_bin_count_max, 0.0f, 255.0f);
            }
  		}
	};
}
