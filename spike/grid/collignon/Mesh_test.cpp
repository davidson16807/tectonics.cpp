...
TEST_CASE( "Mesh.unit_sphere_position() purity", "[rasters]" ) {
    SECTION("Mesh.unit_sphere_position() must be called repeatedly without changing the output"){
        for(int x = -10; x < 10; x+=1){
        for(int y = -10; y < 10; y+=1){
            CHECK(collignon::Mesh(2.0f, 10).unit_sphere_position(glm::vec2(x,y)) == 
                  collignon::Mesh(2.0f, 10).unit_sphere_position(glm::vec2(x,y)));
        }}
    }
}
TEST_CASE( "Mesh unit_sphere_position() closeness preservation", "[rasters]" ) {
    SECTION("changes in grid_position must not result in changes to unit_sphere_position that exceed a reasonable multiple"){
        ...
        for(int y = -10; y < 10; y+=1){
            glm::vec2 v = glm::vec2(x,y);
            CHECK( glm::distance(mesh.unit_sphere_position(v), mesh.unit_sphere_position(v+dx)) < factor * glm::length(dx) );
            CHECK( glm::distance(mesh.unit_sphere_position(v), mesh.unit_sphere_position(v-dx)) < factor * glm::length(dx) );
            CHECK( glm::distance(mesh.unit_sphere_position(v), mesh.unit_sphere_position(v+dy)) < factor * glm::length(dy) );
            CHECK( glm::distance(mesh.unit_sphere_position(v), mesh.unit_sphere_position(v-dy)) < factor * glm::length(dy) );
        }}
    }
}
...
TEST_CASE( "Mesh unit_sphere_position() congruence", "[rasters]" ) {
    SECTION("a modulo can be applied to input which results in the same output"){
        ...
        for(int y = -10; y < 10; y+=1){
            glm::vec2 v = glm::vec2(x,y);
            CHECK( glm::distance(mesh.unit_sphere_position(v), mesh.unit_sphere_position(v+nx)) < epsilon );
            CHECK( glm::distance(mesh.unit_sphere_position(v), mesh.unit_sphere_position(v-nx)) < epsilon );
            CHECK( glm::distance(mesh.unit_sphere_position(v), mesh.unit_sphere_position(v+ny)) < epsilon );
            CHECK( glm::distance(mesh.unit_sphere_position(v), mesh.unit_sphere_position(v-ny)) < epsilon );
        }}
    }
}
    ...
    SECTION("Mesh.unit_sphere_position() must reconstruct input passed to grid_position() for any unit vector"){
        ...
        for(float z = -2.0f; z < 2.0f; z+=0.5f){
            if(glm::length(glm::vec3(x,y,z)) > epsilon){
                ...
                glm::vec2 u = mesh.grid_position(v);
                glm::vec3 v2 = mesh.unit_sphere_position( u );
                CHECK( v2.x == Approx(v.x).margin(epsilon) );
                CHECK( v2.y == Approx(v.y).margin(epsilon) );
                CHECK( v2.z == Approx(v.z).margin(epsilon) );
            }
