...
namespace healpix
{
	...
	*/
    ...
    template<typename id=int, typename scalar=double, glm::qualifier Q=glm::defaultp>
	struct Collignon : IProjection<id,scalar,Q>
	{
    ...
    template<typename id=int, typename scalar=double, glm::qualifier Q=glm::defaultp>
	struct Lambert : IProjection<id,scalar,Q>
	{
	...
	class Projection
	{
		...
		static constexpr scalar total_area = 4*pi;
		static constexpr scalar turn = 2*pi;
	...
	public:
		...
		constexpr Point grid_id(const vec3 sphere_position) const 
		{
			...
			return standardize(Point(i,glm::clamp(V2,s0,s1)));
		}
