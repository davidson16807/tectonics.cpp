...
{
    const Grid& grid;
    const length max_octave_length;
    const scalar hurst_exponent;
    const id octave_count;

    EarthlikeIgneousFormationGeneration(const Grid& grid, const length max_octave_length, const scalar hurst_exponent, const id octave_count):
      grid(grid)
      max_octave_length(max_octave_length),
    ...
    auto operator() (const scalar seed1, const scalar seed2) const
    {
        ...
        return rock::FormationGeneration(grid, 
            field::compose(
                ...
                field::ranked_fractal_brownian_noise<3>(octave_count, hurst_exponent, max_octave_length, seed1, seed2)
            ), 
