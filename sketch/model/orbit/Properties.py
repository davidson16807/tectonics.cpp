...
class Properties:
    ...
    def get_period_from_semi_major_axis(self, semi_major_axis, combined_mass:float):
        a = semi_major_axis
        pi = math.pi
    ...
    # Eccentricity vector
    def get_eccentricity_vector_from_position_and_velocity(self, 
    ...
    def get_semi_latus_rectum_from_momentum_vector(self, momentum_vector: glm.vec3, combined_mass: float):
        mu = self.standard_gravitational_parameter * combined_mass
    ...
    def get_perifocal_velocity(self, semi_latus_rectum: float, eccentricity: float, true_anomaly: float, combined_mass: float):
        P = self.vernal_equinox_direction
        Q = glm.cross(self.north_pole_direction, self.vernal_equinox_direction)
        mu = self.standard_gravitational_parameter * combined_mass
