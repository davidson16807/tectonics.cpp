...
struct ElementsAndState
{
	...
	Elements<scalar> get_elements_from_state(const State<scalar>& state, const scalar combined_mass)
	{
		...
		vec3 node_vector                   = properties.node_vector_from_momentum_vector                        (angular_momentum_vector);
		scalar inclination                 = properties.inclination_from_momentum_vector                        (angular_momentum_vector);
		scalar longitude_of_ascending_node = properties.longitude_of_ascending_node_from_node_vector            (node_vector);
		scalar argument_of_periapsis       = properties.argument_of_periapsis_from_node_and_eccentricity        (node_vector, eccentricity_vector);
		scalar true_anomaly                = properties.true_anomaly_from_position_and_eccentricity             (state.position, eccentricity_vector);
		// scalar argument_of_latitude        = properties.argument_of_latitude_from_position_and_node             (position, node_vector);
		// scalar true_longitude              = properties.true_longitude                                          (longitude_of_ascending_node, argument_of_latitude);
		scalar eccentricity                = glm::length                                                            (eccentricity_vector);
		scalar eccentric_anomaly           = properties.eccentric_anomaly_from_true_anomaly                     (true_anomaly, eccentricity);
		scalar mean_anomaly                = properties.mean_anomaly_from_eccentric_anomaly                     (eccentric_anomaly, eccentricity);
		scalar semi_latus_rectum           = properties.semi_latus_rectum_from_momentum_vector                  (angular_momentum_vector, combined_mass);
		scalar semi_major_axis             = properties.semi_major_axis_from_semi_latus_rectum_and_eccentricity (semi_latus_rectum, eccentricity);
	...
	State<scalar> get_state_from_elements(const Elements<scalar>& elements, const scalar combined_mass)
	{
		...
		scalar eccentric_anomaly = properties.eccentric_anomaly_from_mean_anomaly(elements.mean_anomaly, elements.eccentricity);
		scalar true_anomaly      = properties.true_anomaly_from_eccentric_anomaly  (eccentric_anomaly, elements.eccentricity);
		scalar semi_latus_rectum = properties.semi_latus_rectum_from_semi_major_axis_and_eccentricity (elements.semi_major_axis, elements.eccentricity);
		vec3 perifocal_position  = properties.perifocal_position(semi_latus_rectum, elements.eccentricity, true_anomaly);
