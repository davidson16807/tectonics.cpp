...
struct ElementsAndState
{
	...
	Elements<scalar> get_elements_from_state(const State<scalar>& state)
	{
		...
		vec3 eccentricity_vector           = properties.get_eccentricity_vector_from_position_and_velocity          (state.position, state.velocity);
	...
	State<scalar> get_state_from_elements(const Elements<scalar>& elements)
	{
		...
		vec3 perifocal_velocity  = properties.get_perifocal_velocity(semi_latus_rectum, elements.eccentricity, true_anomaly);
