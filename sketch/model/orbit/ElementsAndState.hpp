...
struct ElementsAndState
{
	...
	const property::Properties<scalar> properties;

	ElementsAndState(property::Properties<scalar> properties): 
		properties(properties)
	{}

	Elements<scalar> get_elements_from_state(const State<scalar>& state)
	{
		glm::vec<3,scalar,glm::defaultp> angular_momentum_vector  = properties.get_angular_momentum_vector_from_position_and_velocity      (state.position, state.velocity);
		glm::vec<3,scalar,glm::defaultp> eccentricity_vector      = properties.get_eccentricity_vector_from_position_and_velocity          (state.position, state.velocity);
		glm::vec<3,scalar,glm::defaultp> node_vector              = properties.get_node_vector_from_momentum_vector                        (angular_momentum_vector);
		...
		scalar semi_latus_rectum           = properties.get_semi_latus_rectum_from_momentum_vector                  (angular_momentum_vector);
	...
	State<scalar> get_state_from_elements(const Elements<scalar>& elements)
	{
