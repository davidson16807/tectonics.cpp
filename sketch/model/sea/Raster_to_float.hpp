...
namespace sea
{
    ...
    void get_volumes(
        ...
        procedural::sub (terrain_height, sea_height,                volumes); // elevation above sea level
        procedural::min (volumes, 0,                                volumes); // negative ocean depth
        procedural::mult(volumes, volumes.grid.cache->vertex_areas, volumes); // negative volume, scaled to unit sphere
        procedural::mult(volumes, -world_radius * world_radius,     volumes); // volumes
    }
    ...
    */
    ...
    float solve_sealevel(
        ...
        float sealevel_min(procedural::min(terrain_height) + uniform_ocean_depth);
        ...
        float sealevel_max(procedural::max(terrain_height) + uniform_ocean_depth);
        ...
        for (unsigned int i = 0; i < iterations; i++) 
        {
            ...
            sea_volume_of_guess = procedural::sum(scratch);
