...
vec3 direction(const float right_ascension, const float declination)
{
   float z = std::sin(declination * si::degree);
   float r = std::cos(declination * si::degree);
   return vec3(
...
int main() {
  ...
  */
  ...
  Spin earth_spin(K, direction(90.0,    66.56 ), K, 23.44*si::degree, 9.2*si::arcsecond,    0.0, 18.6*si::year/s, T/s,      23.93 * si::hour/s, 0.0 * s/s );
  spins.add(3, earth_spin);  // 3 Earth, exaggerated nutation for demonstration
