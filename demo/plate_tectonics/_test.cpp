...
int main() {
  ...
  LithosphereReferenceFrames frames(fine);
  LithosphereSummarization<M,F> summarization(
    rock::crust_summarization<M,F>(),
    crust_summary_ops
  );
  CrustSummaryPredicates predicates(
  ...
  while(!glfwWindowShouldClose(window)) {
      ...
      */
      ...
      for (std::size_t i(0); i < P; ++i)
      {
        ...
        ternary(rifting, procedural::uniform(fresh_column), lithosphere[i], lithosphere[i]);
        ternary(detaching, procedural::uniform(empty_column), lithosphere[i], lithosphere[i]);
      }
